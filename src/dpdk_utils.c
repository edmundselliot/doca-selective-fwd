 ...
 */
static void
print_ether_addr(const struct rte_ether_addr* dmac,
...
print_l2_header(const struct rte_mbuf* packet)
{
    ...
    print_ether_addr(&eth_hdr->dst_addr,
                     &eth_hdr->src_addr,
 ...
 */
static void
print_ipv4_addr(const rte_be32_t dip,
 ...
 */
static void
print_ipv6_addr(const uint8_t dst_addr[16],
...
print_l3_header(const struct rte_mbuf* packet)
{
    if (RTE_ETH_IS_IPV4_HDR(packet->packet_type)) {
        ...
        print_ipv6_addr(ipv6_hdr->dst_addr,
                        ipv6_hdr->src_addr,
                        rte_get_ptype_l4_name(packet->packet_type));
    }
}
 ...
 */
static void
dpdk_chunk_to_mmap_cb(struct rte_mempool* mp,
                      void* opaque,
 ...
 */
static doca_error_t
dpdk_ext_chunk_to_mmap(struct dpdk_mempool_shadow* mempool_shadow,
                       void* chunk,
                       size_t len)
{
    struct doca_mmap* new_mmap;
                                  ...
                                  struct doca_dev* device)
{
    ...
    /* Register each chunk in the mempool to an mmap */
    for (i = 0; i < ext_num; ++i) {
        result = dpdk_ext_chunk_to_mmap(
            mempool_shadow, ext_mem[i]->buf_ptr, ext_mem[i]->buf_len);
        if (result != DOCA_SUCCESS) {
            dpdk_mempool_shadow_destroy(mempool_shadow);
            return NULL;
        }
...
doca_error_t
dpdk_mempool_shadow_find_buf_by_data(struct dpdk_mempool_shadow* mempool_shadow,
...
void
print_header_info(const struct rte_mbuf* packet,
                  const bool l2,
                  const bool l3,
                  const bool l4)
{
