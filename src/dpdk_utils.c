...
static doca_error_t bind_hairpin_queues(uint16_t port_id) {
  ...
  peer_ports_len =
      rte_eth_hairpin_get_peer_ports(port_id, peer_ports, RTE_MAX_ETHPORTS, 1);
  if (peer_ports_len < 0) {
  ...
  peer_ports_len =
      rte_eth_hairpin_get_peer_ports(port_id, peer_ports, RTE_MAX_ETHPORTS, 0);
  if (peer_ports_len < 0) {
...
setup_hairpin_queues(struct application_dpdk_config *app_config,
                     uint16_t port_id, uint16_t peer_port_id,
                     uint16_t *reserved_hairpin_q_list, int hairpin_queue_len) {
  /* Port:
   *	0. RX queue
  ...
  DOCA_LOG_DBG("Setting up hairpin queues %d-%d for %u->%u",
               ...
               reserved_hairpin_q_list[0] + hairpin_queue_len - 1, port_id,
               peer_port_id);
...
static void disable_hairpin_queues(uint16_t nb_ports) {
  ...
  for (port_id = 0; port_id < nb_ports; port_id++) {
    ...
    if (result != DOCA_SUCCESS)
      DOCA_LOG_ERR("Disabling hairpin queues failed: err=%d, port=%u", result,
                   port_id);
  }
}
...
static doca_error_t port_init(struct rte_mempool *mbuf_pool, uint8_t port,
  ...
  if (nb_hairpin_queues) {
    ...
    if (app_config->port_config.self_hairpin &&
      ...
      result = setup_hairpin_queues(app_config, port, port, rss_queue_list,
                                    nb_hairpin_queues / 2);
      if (result != DOCA_SUCCESS) {
        DOCA_LOG_ERR("Cannot hairpin self port %" PRIu8 ", ret: %s", port,
                     doca_error_get_descr(result));
        return result;
      }
      for (queue_index = 0; queue_index < nb_hairpin_queues / 2; queue_index++)
        rss_queue_list[queue_index] = app_config->port_config.nb_queues +
                                      (nb_hairpin_queues / 2) + queue_index;
      result = setup_hairpin_queues(app_config, port, port ^ 1, rss_queue_list,
                                    nb_hairpin_queues / 2);
      if (result != DOCA_SUCCESS) {
        DOCA_LOG_ERR("Cannot hairpin peer port %" PRIu8 ", ret: %s", port ^ 1,
                     doca_error_get_descr(result));
        return result;
      }
    } else {
      ...
      if (rte_eth_dev_is_valid_port(port ^ 1))
        result = setup_hairpin_queues(app_config, port, port ^ 1,
                                      rss_queue_list, nb_hairpin_queues);
      else
        result = setup_hairpin_queues(app_config, port, port, rss_queue_list,
                                      nb_hairpin_queues);
      if (result != DOCA_SUCCESS) {
...
static void print_ipv4_addr(const rte_be32_t dip, const rte_be32_t sip,
  ...
  DOCA_LOG_DBG("DIP=%d.%d.%d.%d, SIP=%d.%d.%d.%d, %s", (dip & 0xff000000) >> 24,
               (dip & 0x00ff0000) >> 16, (dip & 0x0000ff00) >> 8,
               (dip & 0x000000ff), (sip & 0xff000000) >> 24,
               (sip & 0x00ff0000) >> 16, (sip & 0x0000ff00) >> 8,
               (sip & 0x000000ff), packet_type);
}
...
static void print_l4_header(const struct rte_mbuf *packet) {
  ...
  case IPPROTO_UDP:
    ...
    DOCA_LOG_DBG("UDP- DPORT %u, SPORT %u", rte_be_to_cpu_16(udp_hdr->dst_port),
                 rte_be_to_cpu_16(udp_hdr->src_port));
    break;

  case IPPROTO_TCP:
    tcp_hdr = (typeof(tcp_hdr))l4_hdr;
    DOCA_LOG_DBG("TCP- DPORT %u, SPORT %u", rte_be_to_cpu_16(tcp_hdr->dst_port),
...
doca_error_t dpdk_mempool_shadow_find_buf_by_data(
  ...
  for (mmap_idx = 0; mmap_idx < mempool_shadow->nb_mmaps; mmap_idx++) {
    ...
    if (mmap_begin <= mem_range_start &&
      ...
      return doca_buf_inventory_buf_get_by_data(
          inventory, mmap, (void *)mem_range_start, mem_range_size, out_buf);
  }
