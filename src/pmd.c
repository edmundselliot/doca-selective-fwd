    ...
    while (1) {
        for (int port_id_in = 0; port_id_in < NUM_PORTS; port_id_in++) {
            for (int queue_id = 0; queue_id < nb_queues; queue_id++) {
                int nb_packets = rte_eth_rx_burst(
                    port_id_in, queue_id, packets, PACKET_BURST_SZ);
                if (nb_packets == 0) {
                    continue;
                }

                for (int packet_idx = 0; packet_idx < nb_packets;
                     packet_idx++) {
                    ...
                    printf("[P%d Q%d] Allow %s:%d <-> %s:%d? (s/p/n): ",
                           port_id_in,
                           queue_id,
                           src_addr,
                    ...
                    if (result != DOCA_SUCCESS) {
                        DOCA_LOG_ERR(
                            "Failed to add main hairpin pipe entry: %s",
                            doca_error_get_descr(result));
                        continue;
                    }

                    rte_eth_tx_burst(
                        dest_port, queue_id, &packets[packet_idx], 1);
