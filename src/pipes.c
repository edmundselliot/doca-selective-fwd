...
static doca_error_t create_rss_pipe(struct doca_flow_port *port,
  ...
  /* Match on any packet */
  result = doca_flow_pipe_add_entry(0, *pipe, &match, NULL, NULL, &fwd, 0,
                                    &status, NULL);
  if (result != DOCA_SUCCESS) {
 ...
 */
doca_error_t
add_hairpin_pipe_entry(struct doca_flow_port *ports[NUM_PORTS], int port_id_in,
                       uint16_t base_hairpin_q, uint8_t hairpin_q_len,
                       struct doca_flow_pipe *pipe, doca_be32_t dst_ip_addr,
                       doca_be32_t src_ip_addr, doca_be16_t dst_port,
  ...
  result = doca_flow_pipe_add_entry(0, pipe, &match, &actions, NULL, &fwd, 0,
                                    &status, &entry);
  if (result != DOCA_SUCCESS) {
...
configure_static_pipes(struct application_dpdk_config *app_cfg,
  ...
  for (int port_id = 0; port_id < NUM_PORTS; port_id++) {
    ...
    result = create_rss_pipe(ports[port_id], &rss_pipes[port_id],
    ...
    result = create_hairpin_pipe(ports[port_id], port_id, rss_pipes[port_id],
