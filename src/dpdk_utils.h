...
struct application_port_config {
  ...
  uint16_t nb_queues; /* Set on init to 0 for don't care, required minimum cores
                         otherwise */
  ...
  uint16_t
      self_hairpin : 1; /* Set on init to 1 enable both self and peer hairpin */
  uint16_t rss_support : 1;  /* Set on init to 0 for no RSS support, RSS support
                                otherwise */
...
struct application_dpdk_config {
  ...
  // range will be from hairpin_queues[x][y] to hairpin_queues[x][y] +
  // hairpin_q_count - 1
